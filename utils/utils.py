...
def make_csv(df, args):
    df.to_csv(f'{args.csv_path}{args.model}_{args.dataset}_{args.choice_data}_lr{args.lr}_wd{args.wd}_seq{args.seq_len}_step{args.step_len}_batch{args.batch_size}_epoch{args.epoch}_score{args.score}_calc{args.calc}_num_stack_{args.stack_num}.csv', header = True, index = False)
    # if args.dataset == 'NAB':
